...
builder.Services.AddOpenTelemetry()
    ...
    {
        tracing
            .SetResourceBuilder(ResourceBuilder.CreateDefault().AddService("NetIAM.AdminApi"))
...
{
    options.AddFixedWindowLimiter("admin-fixed-window", configure =>
    {
        configure.PermitLimit = 120;
        configure.Window = TimeSpan.FromMinutes(1);
        configure.QueueProcessingOrder = System.Threading.RateLimiting.QueueProcessingOrder.OldestFirst;
        configure.QueueLimit = 30;
    });
});
